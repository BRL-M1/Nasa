...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@BRL_M1"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : بكر"
    )
...
async def _(event):
    ...
    6364281889 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"6364281889 : `{event.sender_id}`")
    else:
